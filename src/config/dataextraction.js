...
const exportToS3 = require('../config/s3');
...
module.exports = {
  ...
  newExtractBusinessCardInfo: async (imagedata, info,imageBuffers) => {
    try {
      // Validate input
      if (!imagedata || !Array.isArray(imagedata) || imagedata.length === 0) {
        throw new Error('No image data provided');
      ...
      const json = JSON.parse(cleaned);
      console.log("-------",json);
      const s3result = await exportToS3.exportToS3(imageBuffers, json['Company Name']);
      console.log("s3result", s3result);
      if (s3result.success==false) throw errorUtils.createError(s3result.error, 400);
      const insertImage = await cardmodel.insertcardbase65([s3result.frontUrl,s3result.backUrl],info);   

      if (insertImage.success != 1) throw errorUtils.createError(insertImage.msg, insertImage.success || 400);
      
      // Insert the extracted data
      await cardmodel.insertextracteddata(json, insertImage.data['cardid'], info);
      
      console.log(`Successfully processed ${parts.length - 1} image(s) and extracted data`);
      const newjson = await cardmodel.getcardlistsbyid(insertImage.data['cardid']);
      // console.log("==== ",newjson);
